...
5. Checks the mix for long silent gaps (auto-fails if found)
...
def generate_and_download_one(index: int) -> Path | None:
    ...
    print(f"[{index:03d}] requesting generation: {title}")
    proc = subprocess.run(cmd, capture_output=True, text=True, cwd="tunee-skill")
    ...
    print(f"[{index:03d}] waiting {GENERATION_WAIT_SECONDS}s for generation to finish...")
    ...
    try:
        ...
        if not mp3_urls:
            print(f"[{index:03d}] no mp3 URL found on share page")
            return None
        ...
        if audio_resp.status_code != 200 or len(audio_resp.content) < 10_000:
            print(f"[{index:03d}] download failed or file too small "
                  f"(status={audio_resp.status_code}, bytes={len(audio_resp.content)})")
            return None

        out_path = TRACKS_DIR / f"track_{index:03d}.mp3"
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_bytes(audio_resp.content)
        print(f"[{index:03d}] downloaded OK ({len(audio_resp.content)} bytes)")
        return out_path

    except requests.RequestException as e:
        print(f"[{index:03d}] network error: {e}")
        return None
...
def main():
    ...
    while len(successful_tracks) < TARGET_TRACKS:
        ...
        if track_path is not None:
            ...
            if consecutive_failures >= MAX_CONSECUTIVE_FAILURES:
                ...
                print(f"ABORTING: {detail}")
                sys.exit(1)

    print(f"\nAll {len(successful_tracks)} tracks downloaded successfully.")

    # --- Phase 2: crossfade into one mix ---
    print("[mix] combining tracks with crossfade...")
    mix = build_crossfaded_mix(successful_tracks)

    # --- Phase 3: silence check ---
    silent_ranges = detect_silence(mix, min_silence_len=MAX_SILENCE_MS, silence_thresh=SILENCE_THRESH_DB)
    if silent_ranges:
        detail = f"ミックスの生成に失敗しました({len(silent_ranges)}箇所の無音区間を検出、リトライ上限到達)"
    ...
    print(f"[OK] exported {MIX_PATH} ({duration_min:.1f} minutes)")
